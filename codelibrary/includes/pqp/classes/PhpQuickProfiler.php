...
class PhpQuickProfiler {
	...
	public $output = array();
	public $config = __SITE_PATH;
	private $gMysql = null;

#	public function __construct($startTime, $config = "/app/includes/pqp/") {
	public function __construct($config = "/includes/pqp/") {
		$this->startTime	=	$this->getMicroTime();
		$this->config		=	$config;
		require_once('Console.php');
	}

	public function setup($gMysql)
	{
		$this->gMysql		=	$gMysql;
	}
	...
	public function gatherQueryData() {
		...
		if	(!empty($this->gMysql))
		{
			$queryTotals['count'] = $this->gMysql->queries;

			foreach($this->gMysql->queriesArray as $key => $query)
			{

#AddGameComment("key:$key: query:$query");
	...
	function attemptToExplainQuery($query) {
		...
		if	(stristr($query['sql'],"select") !== FALSE)
		{
	...
	public function display($db = '', $master_db = '') {
		...
		require_once($_SERVER["DOCUMENT_ROOT"]	.	"/../codelibrary/includes/pqp/display.php");
